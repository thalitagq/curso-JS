  ...
  */
  ...
  $numbers.forEach(function(item){
    return item.addEventListener('click', function (event) {
      $input.value === '0' 
      ? $input.value = item.dataset.js 
      : insert(item.dataset.js);
    })  
  })
  function insert(item){
    var addItem = '';
    var inputAux = $input.value;
    switch (item) {
      case 'add':
        addItem = '+';
        break;
    ...
    'sad'.slice()
    if (!isNumber($input.value.substring($input.value.length - 1)) && !isNumber(item)) {
  ...
  function evaluate(){
    ...
    console.log('evaluate', $input.value);
