(function () {
  ...
  */
  ...
  var exp = /ju\w{3}/g;
  console.log('Os meses de janeiro, junho e julho começam com a letra j.'.match(exp));
  ...
  */
  ...
  var tag = /<\w+>/g;
  console.log("<div><section><blockquote>Texto <img /></blockquote></section></div>".match(tag));
  ...
  */
  ...
  var tag2 = /<\w+><\/\w+>/g;
  console.log("<div><ul><li></li><li></li><li><span></span></li></ul></div>".match(tag2));
  ...
  */
  ...
  console.log(
    "<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>".replace(/<(\w+)>([^<]+)<\/(\w+)>/g, function (fullTag, open, text, close) {
      return '<' + open + '>' + 'O texto dentro da tag '+ '"' + open + '"' + ' é ' + '"'+ text + '"' + '</' + close + '>\n';
    })
  );
})()
